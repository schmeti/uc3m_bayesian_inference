...
prob_data_rec <- data |>
  ...
  recipe() |>
  step_zv(everything()) |> # zero variance
  step_nzv(everything()) |> # near zero variance
  ...
  step_lincomb(everything()) |> # exact linear combination between them 
  prep()
prob_data_rec
# TODAS LAS QUE SACA ES POR CORRELACIÓN
prob_data_rec[["steps"]][[4]][["removals"]]
vars_to_exclude
```
...
# Check correlation between numeric variables
library(corrplot)
cor_matrix <- cor(data[, setdiff(numeric_vars, vars_to_exclude)])
corrplot(cor_matrix, method = "circle")

# Or find highly correlated variables
high_cor_f <- findCorrelation(cor_matrix, cutoff = 0.8)
problematic_vars_f <- numeric_vars[high_cor_f]
...
DIC = function(model, data) {
  ...
  for (i in 1:nrow(model)) {
    ...
    p[data$diagnosis == 0] = 1-p[data$diagnosis == 0]
  ...
  p_post[data$diagnosis == 0] = 1-p_post[data$diagnosis == 0]
